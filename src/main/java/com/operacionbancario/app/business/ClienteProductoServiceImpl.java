...
import com.operacionbancario.app.util.Constante;
...
/*
	...
	public Mono<ClienteProductosBancarioDTO> saveClienteProductoDTO(ClienteProductosBancarioDTO dto) {
		if(dto.getCliente()!= null && dto.getProductosBancarios().size()> 0 && dto.getProductosBancarios() != null) {
			...
			.subscribe();
			dto.getProductosBancarios().forEach(x -> {
				WebClient.builder().baseUrl("http://localhost:8099/micro-bancario/productos/").build()
				.get().uri(x.getIdProducto()).retrieve().bodyToMono(ProductoBancario.class).log()
				.map(p-> {
					ClienteProductoBancario clproducto = new ClienteProductoBancario(dto.getCliente(), p);
					clienteProductoRepo.save(clproducto).subscribe();
					cuentaRepo.save(new CuentaBancaria(dto.getCliente())).subscribe();
					return p;
				}).subscribe();
			});
		}
	...
	public Flux<CuentaBancaria> saveClienteProductoDTOv3(ClienteProductosBancarioDTO dto) {
		...
		.flatMapMany(cl -> {
			...
			.get().uri(pro.getIdProducto()).retrieve().bodyToMono(ProductoBancario.class).log();
			})
		.flatMap(pro -> {
			ClienteProductoBancario clproducto = new ClienteProductoBancario(dto.getCliente(), pro);
			clienteProductoRepo.save(clproducto).subscribe();
			return cuentaRepo.save(new CuentaBancaria(dto.getCliente()));
		}).collectList().flux()
		.flatMap(cs -> {
